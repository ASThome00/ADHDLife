...
export function HabitCard({ habit, doneDates }: Props) {
  ...
  const message = paused
    ? `Paused at ${habit.longest_streak} days — start again today!`
    ...
    <div className="card" style={{ marginBottom: 14, position: 'relative' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
        ...
        >
          <div className={cn('hcircle', habit.completed_today && 'done')}>
            {habit.completed_today ? (
              ...
              <span style={{ width: 14, height: 14, borderRadius: '50%', background: habit.color, display: 'inline-block', opacity: 0.8 }} />
            )}
          </div>
        </button>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {editing ? (
            <input
              ...
              style={{ fontFamily: 'Lora, serif', fontWeight: 600, fontSize: 15, color: 'var(--text-primary)', background: 'transparent', border: 'none', borderBottom: '1.5px solid var(--accent)', outline: 'none', padding: '2px 0', width: '100%' }}
            />
          ) : (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ width: 9, height: 9, borderRadius: '50%', background: habit.color, flexShrink: 0 }} />
              <span style={{ fontFamily: 'Lora, serif', fontWeight: 600, fontSize: 15, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {habit.name}
              </span>
            </div>
          )}
          <div style={{ fontFamily: 'Lora, serif', fontStyle: 'italic', fontSize: 12, color: 'var(--text-mono)', marginTop: 3 }}>
            {message}
          </div>
        </div>

        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 16, fontWeight: 500, color: habit.current_streak > 0 ? 'var(--accent)' : 'var(--text-faint)', whiteSpace: 'nowrap' }}>
            🔥 {habit.current_streak} day{habit.current_streak === 1 ? '' : 's'}
          </div>
          <div style={{ fontFamily: 'Geist, sans-serif', fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>
        ...
        <button
          ...
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-faint)', fontSize: 16, padding: '2px 6px', flexShrink: 0, lineHeight: 1 }}
        >
          ⋯
        </button>
      </div>
      ...
      {menuOpen && (
        <>
          ...
          <div style={{ position: 'absolute', top: 42, right: 14, zIndex: 91, background: 'var(--bg-card)', border: '1.5px solid var(--border)', borderRadius: 10, boxShadow: '3px 4px 0 var(--shadow)', padding: 4, minWidth: 130 }}>
...
function MenuItem({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      style={{ display: 'block', width: '100%', textAlign: 'left', padding: '7px 10px', borderRadius: 7, border: 'none', background: 'transparent', fontFamily: 'Geist, sans-serif', fontSize: 12.5, color: 'var(--text-body)', cursor: 'pointer' }}
